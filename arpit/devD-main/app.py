...
from interview_module import show_interview_practice_page
...
try:
    ...
    st.warning(f"Database unavailable; continuing in limited mode. Details: {exc}")
...
def main():
    ...
    # Main content
    tab1, tab_interview, tab3, tab4 = st.tabs(
        ["🎯 Match Me to Job", "🤖 AI Interview Practice", "📊 Analytics", "💳 Billing"]
    )
...
def show_interview_qa_page():
    """
    AI Interview Practice page — full practice session with AI evaluation,
    voice recording, structured Q&A, and downloadable feedback report.
    Also includes a legacy Q&A bank download option.
    """
    # Sub-tabs: Practice Session | Quick Q&A Download
    sub_practice, sub_qa = st.tabs(
        ["🤖 AI Interview Practice Session", "📥 Quick Q&A Download"]
    )

    with sub_practice:
        # Delegate fully to the interview module
    ...
    with sub_qa:
        st.markdown("## 📥 Quick Interview Q&A Download")
        st.markdown(
            "Generate and download a complete Q&A bank (PDF + DOCX) "
            "without entering a live practice session. **Costs 3 credits.**"
        )
        ...
        st.button("🧹 Clear JD", help="Click to clear job description",
                  on_click=clear_jd_tab2, key="clear_jd_tab2")
        ...
        if uploaded_resume_tab2:
            ...
            with st.expander("📄 Resume Preview"):
                st.text_area("Resume Content", resume_text_preview[:2000],
                             height=300, disabled=True)

        if jd_tab2.strip() and uploaded_resume_tab2:
            if st.button("🎤 Generate Interview Q&A (3 credits)", key="generate_qa_tab2"):
                if not check_user_access(required_credits=3):
                    st.error("⚠️ Insufficient credits. Please purchase more.")
                    return
                ...
                loading_placeholder.markdown("""
                    <div style="display:flex;flex-direction:column;align-items:center;padding:20px">
                        <div class="custom-loader"></div>
                        <p style="margin-top:10px;font-weight:bold;font-size:16px">
                            ⏳ Generating interview Q&A... Please wait
                        </p>
                    </div>
                """, unsafe_allow_html=True)
                ...
                try:
                    ...
                    with col1:
                        st.download_button(
                            ...
                            file_name="interview_QA.pdf", mime="application/pdf",
                            key="download_pdf_tab2"
                        )
                    with col2:
                        st.download_button(
                            "📥 Download DOCX", data=docx_buffer,
                            file_name="interview_QA.docx",
                            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    ...
                    deduct_user_credits(
                        st.session_state.user_data['email'], 3, feature="Interview QA"
                    )
